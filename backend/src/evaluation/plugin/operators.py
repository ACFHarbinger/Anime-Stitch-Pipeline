...
``animation.evaluation.plugin`` has to stay importable without it. The
...
def _bootstrap() -> None:
    ...
    *not* as part of ``animation.evaluation.plugin`` — so relative
...
from animation.evaluation.other.metrics_view import (  # noqa: E402
    RADAR_SCALES,
    radar_value,
)
...
class SyncEvaluations(foo.Operator):
    ...
    def execute(self, ctx):
        from animation.evaluation.plugin import sync
